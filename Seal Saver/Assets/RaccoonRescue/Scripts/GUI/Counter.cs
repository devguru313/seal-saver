...
public class Counter : MonoBehaviour
{
	...
	void Update()
	{
		if (name == "Moves") {
            ...
            if (LevelData.LimitAmount == 1 && GameEvent.Instance.GameStatus == GameState.Playing)           //Stop asking questions if last move
            {
                mainscript.isGameOver = true;
            }
            if (LevelData.LimitAmount > 1 && GameEvent.Instance.GameStatus == GameState.Playing)            //Start asking questions on level restart
            {
