...
public class GameEvent : MonoBehaviour
{
	...
	IEnumerator WinAction()
	{
		...
		if (PlayerPrefs.GetInt(string.Format("Level.{0:000}.StarsCount", mainscript.Instance.currentLevel), 0) < mainscript.Instance.stars && (Login.subscribed || mainscript.Instance.currentLevel < 2))
			PlayerPrefs.SetInt(string.Format("Level.{0:000}.StarsCount", mainscript.Instance.currentLevel), mainscript.Instance.stars);


		if (PlayerPrefs.GetInt("Score" + mainscript.Instance.currentLevel) < mainscript.Score && (Login.subscribed || mainscript.Instance.currentLevel < 3)) {
    ...
    public static void ShowQuestion ()              //Show Question UI
    {
        MenuManager.Instance.ShowQuestion();
        QuestionManager.askTime = System.DateTime.Now;
    }
